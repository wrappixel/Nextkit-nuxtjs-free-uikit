export function formatCurrency(price = 0, locales = 'vi') {
  const currentLocale = localStorage.locale ? localStorage.locale : locales
  const type = getCurrencyByLocales(currentLocale)
  const currency = new Intl.NumberFormat(currentLocale, { style: 'currency', currency: type, maximumFractionDigits: 3 }).format(price)
  return currency
}
